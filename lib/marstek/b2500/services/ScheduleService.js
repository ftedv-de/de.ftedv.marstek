...
function assertTargetPower(watts) {
  const value = Number(watts);
  ...
  if (value < -2500) {
    throw new Error('Target power is below the supported discharge range');
  }
...
function updateHomeyPowerSlot(currentSlots, targetPower) {
  const normalizedTargetPower = assertTargetPower(targetPower);
  const slots = normalizeSlots(currentSlots);
  const power = Math.abs(normalizedTargetPower);
