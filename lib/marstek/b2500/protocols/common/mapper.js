...
function toZeroWhenMissing(value) {
...
function mapState(d) {
  ...
  const chargePower = pvPower;
  const dischargePower = outputPower;
  const netBatteryPower = toZeroWhenMissing(dischargePower) - toZeroWhenMissing(chargePower);
  const scheduleSlots = parseScheduleSlots(d);

  return {
    measure_battery: toNumber(d.pe),
    measure_power: netBatteryPower,
    ...
    b2500_charge_power: chargePower,
    b2500_discharge_power: dischargePower,
